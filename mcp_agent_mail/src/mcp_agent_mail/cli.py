...
try:
    # Optional at import-time: MAF shell scripts import AgentMailClient from this module
    # without installing the full server dependency stack (e.g. fastmcp).
    from .app import _sanitize_fts_query
except Exception:
    def _sanitize_fts_query(query: str) -> str:  # type: ignore[no-redef]
        return query
...
# Suppress noisy dependency warnings when this module is imported by shell scripts.
...
class AgentMailClient:
    """
    Minimal JSON-RPC client used by local MAF shell scripts.

    Notes:
    - This intentionally provides a tiny API surface (`send_message`, `fetch_inbox`)
      to maintain backwards compatibility with existing scripts that do
      `from mcp_agent_mail.cli import AgentMailClient`.
    - Scripts historically omitted recipients; when `to` is not provided we infer
      a recipient agent name from common thread_id patterns (e.g. CONTEXT-<Agent>-...).
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._server_url = server_url or f"http://{settings.http.host}:{settings.http.port}{settings.http.path}"
        self._bearer_token = bearer_token or (settings.http.bearer_token or "")
    ...
    def _call_tool(self, name: str, arguments: dict[str, Any]) -> Any:
        req = {
            "jsonrpc": "2.0",
            "id": f"cli-AgentMailClient-{name}",
            "method": "tools/call",
            "params": {"name": name, "arguments": arguments},
        }
        with httpx.Client(timeout=self._timeout_seconds) as client:
            resp = client.post(self._server_url, json=req, headers=self._headers())
        ...
        if isinstance(result, dict):
            ...
            # Fall back to attempting to parse the first text content as JSON.
            content = result.get("content")
            if isinstance(content, list) and content:
                first = content[0]
                if isinstance(first, dict) and isinstance(first.get("text"), str):
    ...
    def _is_valid_agent_name(self, name: str) -> bool:
        # Server enforces adjective+noun CamelCase.
        return bool(re.match(r"^[A-Z][a-z]+[A-Z][a-z]+$", name))

    def _infer_to_agents(self, thread_id: Optional[str]) -> list[str]:
        if not thread_id:
            return []
        # Common patterns used by scripts:
        # - CONTEXT-<Agent>-<pane>
        # - CONTEXT-<Agent>-<pane_id> (session_name is usually the agent)
    ...
    def _ensure_agent(self, *, project_key: str, agent_name: str) -> None:
        try:
            self._call_tool(
                "register_agent",
                {
                    ...
                    "model": "n/a",
                    "name": agent_name,
                    "task_description": "MAF script (auto-registered)",
                },
            )
        except Exception:
            # Best-effort: if registration fails (already exists or server disallows),
            # still attempt to send/fetch with the provided name.
            pass
    ...
    def send_message(
        ...
        *,
        ...
        if not self._is_valid_agent_name(sender_name):
            # MAF scripts historically pass non-agent identifiers like "context-manager".
            # Use the primary recipient as the sender to satisfy server agent-name rules.
            sender_name = to_agents[0] if to_agents else sender_name

        self._ensure_agent(project_key=project_key, agent_name=sender_name)
        for recipient in to_agents:
            self._ensure_agent(project_key=project_key, agent_name=recipient)
    ...
    def fetch_inbox(
        ...
        *,
        ...
        self._ensure_agent(project_key=project_key, agent_name=agent_name)
        ...
        # Normalize shape for older scripts.
...
def serve_http(
    ...
    try:
        ...
        console.print(f"[red]Cannot import server dependencies (fastmcp): {e}[/]")
